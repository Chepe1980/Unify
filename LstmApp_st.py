...
# Configure the app
st.set_page_config(
    page_title="Well Log Prediction with Neural Network",
    page_icon="⛏️",
    layout="wide",
...
def main():
    st.title("⛏️ Well Log Prediction using Neural Network")
    st.markdown("""
    This app predicts well log measurements from other well log data using a Neural Network.
    ...
    with st.sidebar:
        ...
        if uploaded_file:
            try:
                ...
                target_curve = st.selectbox("Target Curve", available_curves, 
                                          index=0)
                
                # Default input curves
                default_curves = ['GR', 'RT', 'NPHI', 'RHOB', 'PEF', 'DTS']
                input_curves = []
                
                for i in range(6):  # Reduced to 6 input curves for simplicity
                    curve = st.selectbox(
                        f"Input Curve {i+1}", 
                        available_curves,
    ...
    with st.spinner("🔄 Processing data and training model..."):
        try:
            ...
            st.session_state.predictions = {
                ...
                'units': st.session_state.original_las.curves[target_curve].unit if hasattr(st.session_state.original_las.curves[target_curve], 'unit') else ''
            }
...
def display_results():
    ...
    units = results['units'] if results['units'] else ""
    ...
    with col2:
        ...
        <div class="metric-card">
            ...
            <p>Final Training Loss: <strong>{results['history'].history['loss'][-1]:.4f}</strong></p>
            <p>Final Validation Loss: <strong>{results['history'].history['val_loss'][-1]:.4f}</strong></p>
        </div>
        """, unsafe_allow_html=True)
    ...
    scatter = ax2.scatter(
        ...
        vmin=np.min(results['depth']),
        vmax=np.max(results['depth'])
    )
    ax2.plot([results['y_test'].min(), results['y_test'].max()],
             [results['y_test'].min(), results['y_test'].max()],
             'r--', label='1:1 Line')
    ax2.set_xlabel(f'Actual {results["target_curve"]} ({units})')
    ax2.set_ylabel(f'Predicted {results["target_curve"]} ({units})')
    ax2.set_title(f'{results["target_curve"]} Prediction (R² = {results["r2"]:.3f})')
    ...
    ax3.plot(results['y_test'], results['depth'], 'b-', label=f'Actual {results["target_curve"]}', linewidth=1)
    ax3.plot(results['y_pred'], results['depth'], 'r--', label=f'Predicted {results["target_curve"]}', linewidth=1)
    ax3.invert_yaxis()
    ax3.set_xlabel(f'{results["target_curve"]} ({units})')
    ax3.set_ylabel('Depth')
    ax3.set_title(f'{results["target_curve"]} Comparison Along Depth')
    ax3.legend()
    st.pyplot(fig3)
    ...
    ax4a.set_xlabel(f'Residual (Actual - Predicted) ({units})')
    ...
    ax4b.set_xlabel(f'Predicted {results["target_curve"]} ({units})')
    ax4b.set_ylabel(f'Residual ({units})')
    ...
    df = pd.DataFrame({
        'DEPTH': results['depth'],
        f'{results["target_curve"]}_ACTUAL': results['y_test'].flatten(),
        f'{results["target_curve"]}_PREDICTED': results['y_pred'].flatten(),
    ...
    try:
        ...
        # Add predicted curve (need to align with original depth)
        ...
        new_las.add_curve(
            ...
            full_pred,
            unit=results['units'],
            descr=f"Predicted {results['target_curve']} Log"
        )
    ...
    with col1:
        st.download_button(
            ...
            file_name=f"{results['target_curve']}_predictions.csv",
            mime="text/csv",
    ...
    with col2:
        st.download_button(
            ...
            file_name=f"{results['target_curve']}_predictions.las",
