...
def main():
    ...
    with st.sidebar:
        ...
        if uploaded_file:
            try:
                ...
                st.session_state.original_las = las  # Store original LAS for later
                available_curves = [curve for curve in las.keys() if curve != 'DEPTH']
                
                st.header("⚙️ Model Parameters")
                epochs = st.slider("Number of Epochs", 10, 200, 50, help="Number of training iterations")
    ...
    if uploaded_file and not st.session_state.model_trained:
        try:
            las_df = las.df()
            ...
            stats_df = las_df.describe()
            st.dataframe(stats_df, use_container_width=True)
            
            # Quick plot of all curves
            st.subheader("📈 Quick View of All Curves")
            fig, ax = plt.subplots(figsize=(12, 8))
            las_df.plot(subplots=True, ax=ax)
            st.pyplot(fig)
        except Exception as e:
